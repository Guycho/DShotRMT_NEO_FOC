...
RMT_CALLBACK_ATTR
static bool rx_done_callback(rmt_channel_handle_t channel, const rmt_rx_done_event_data_t *edata,
  void *user_data) {
    // init return value
    // high task wakeup will be true if pushing data to the queue started a task with a higher
    // priority than this inturrupt.
    ...
    size_t sym_count = edata->num_symbols > RX_SYMBOL_MAX
                         ? RX_SYMBOL_MAX
                         : edata->num_symbols;  // cap copy size to 11
    memcpy(&out_data.received_symbols, edata->received_symbols,
    ...
    // we're getting a packet back whose last symbol does not end in the terminating value (which is
    // a symbol with 0 length values)
    ...
    if (edata->received_symbols[last_sym].duration0 != 0 &&
        ...
        // this results in a guru meditation error, but we shouldn't be getting these anyway... I
        // wonder what the packet looks like... Serial.printf("~\n"); out_data.non_termination =
        // true;
    }
    ...
    return high_task_wakeup ==
           pdTRUE;  // return xQueueSendFromISR result (does it wake up a higher task?)
}

RMT_CALLBACK_ATTR
static bool tx_done_callback(rmt_channel_handle_t tx_chan, const rmt_tx_done_event_data_t *edata,
  void *user_ctx) {
    BaseType_t high_task_wakeup = pdFALSE;
    ...
    // enable open drain mode before listening for a response (no need for snap logic here, we only
    // do callbacks in bidirectional mode)
    gpio_ll_od_enable(GPIO_LL_GET_HW(GPIO_PORT_0), config->gpio_num);

    // start listening for a response
    rmt_receive(config->channel_handle, config->raw_symbols, config->raw_sym_size,
...
static const unsigned char GCR_encode[16] = {0x19, 0x1B, 0x12, 0x13, 0x1D, 0x15, 0x16, 0x17, 0x1A,
  0x09, 0x0A, 0x0B, 0x1E, 0x0D, 0x0E, 0x0F};
...
void DShotRMT::begin(dshot_mode_t dshot_mode, bidirectional_mode_t is_bidirectional,
...
void DShotRMT::begin(uint8_t pin, dshot_mode_t dshot_mode, bidirectional_mode_t is_bidirectional,
  uint16_t magnet_count) {
    dshot_timer = new Chrono(Chrono::MICROS);
    dshot_config.gpio_num = (gpio_num_t)pin;
    ...
    // setup dshot timings
    {
        switch (dshot_config.mode) {
            case DSHOT150:
                ...
                // dshot_config.micros_per_frame = 106720 * 110 / 100; //dshot specs are at 106720
                // (110% on timing: 117392) dshot_config.micros_per_shortest =
                // dshot_config.ticks_zero_high * 90/100; //
            ...
            case DSHOT300:
                ...
                // dshot_config.micros_per_frame = 58608; //dshot specs are at 53280 (110% on
                // timing: 58608)
                break;
            ...
            case DSHOT600:
                ...
                // dshot_config.micros_per_frame = 29392; //dshot specs are at 26720 (110% on
                // timing: 29392)
                break;
            ...
            case DSHOT1200:
                ...
                // dshot_config.micros_per_frame = 14608; //dshot specs are at 13280 (110% on
                // timing: 14608)
                break;
        ...
        // note that the ESC responds with a slightly higher bitrate of 5/4, so timing issues may
        // happen due to my nasty math here (probably not though) frame count * (ticks / 10) * 1000
        // for ms
        dshot_config.micros_per_frame =
          16 * dshot_config.ticks_per_bit * 110;  // 110/100: 110% longer
    ...
    // we have 2 different if statements with the same condition because enabling loopback is order
    // sensitive the RX channel must be installed first
    if (dshot_config.bidirectional) {
        ...
        rmt_rx_channel_config_t rx_chan_config = {.gpio_num = dshot_config.gpio_num,
          .clk_src =
            RMT_CLK_SRC_DEFAULT,  // select a clock that can provide needed resolution (apb)
          .resolution_hz = RMT_CYCLES_PER_SEC,  // esc resolution in Hz
          .mem_block_symbols = 64,              // default count per channel
          .flags = {
        ...
        rmt_rx_event_callbacks_t callback = {.on_recv_done = rx_done_callback};
        ...
        // register them
        handle_error(rmt_rx_register_event_callbacks(dshot_config.rx_chan, &callback,
          &dshot_config.rx_callback_datapack));
        ...
        dshot_config.tx_callback_datapack.raw_sym_size =
    ...
    if (dshot_config.bidirectional) {
        ...
        rmt_tx_event_callbacks_t callback = {.on_trans_done = tx_done_callback};

        // tx event will be set on the TX channel, with "callback" config and fed the timing config
        // for the RX reception RX reception will be enabled as soon as the TX transmission is over
        handle_error(rmt_tx_register_event_callbacks(dshot_config.tx_chan, &callback,
          &dshot_config.tx_callback_datapack));

        // enable RX channel (may need to remove this, we disable the channel right away when we go
        // to send a packet)
    ...
    // make a copy encoder to push out the array of rmt_symbol_word_t we make when we process a
    // dshot frame
...
dshot_send_packet_exit_mode_t DShotRMT::send_dshot_value(uint16_t throttle_value,
  telemetric_request_t telemetric_request) {
    ...
    dshot_frame.telemetry = telemetric_request;  // NOT bidirectional telemetry (requests telemetry
                                                 // over second wire, which is not implemented)
    ...
    // rmt_ll_rx_enable((rmt_dev_t*)hal->regs, dshot_config.rx_chan.channel_id, false); //ALTER
    // (cast)
    ...
    if (dshot_config.bidirectional == ENABLE_BIDIRECTION) {
        ...
        switch (err) {
            ...
            default: {
                handle_error(err);
                return ERR_RMT_DISABLE_FAILURE;  // don't try to send out the RMT signal if we get
                                                 // an error
            }
        }
    }
    ...
    rmt_transmit(dshot_config.tx_chan, dshot_config.copy_encoder, &dshot_tx_rmt_item,
      sizeof(rmt_symbol_word_t) * DSHOT_PACKET_LENGTH, &tx_config);

    return SEND_SUCCESS;
}

/// pre-encode a dshot packet to send repeatedly with `send_last_value`
void DShotRMT::prepare_dshot_value(uint16_t throttle_value,
  telemetric_request_t telemetric_request) {
    dshot_esc_frame_t dshot_frame = {};
...
FAST_ATTR_FLAG dshot_get_packet_exit_mode_t DShotRMT::get_dshot_packet(uint32_t *value,
  extended_telem_type_t *packetType) {
    if (dshot_config.bidirectional != ENABLE_BIDIRECTION) {
    ...
    if (
      ...
      xQueueReceiveFromISR(receive_queue, &rx_data, NULL)
        ...
        if (rx_data.num_symbols > 1) {
            ...
            // with 600, recieved packets can have veriety that simultaneously renders the 90%
            // reduction needed and erronious (we get ticks with time 30 and time 40. Where do these
            // go?)
            ...
            for (i = 0; i < rx_data.num_symbols; ++i) {
                // due to how the GCR encoding works, there will never be more than 3 of the same
                // bit in a row. We can assume all bits with length over a threshhold are 3.
            ...
            // If we invert the input logic with RMT, we don't need to do this (esc response sends
            // inverted logic, a HIGH == 0) spaces come pre-filled with 0 fill any remaining space
            // with '1's
            ...
            if (crc != alsocrc) {
                error_packets += 1;  // for now, the only error packets we will track are the ones
                                     // where the checksum fails
            ...
            if (frameData & 0b000100000000 ||
                (~frameData & 0b111100000000) ==
                  0b111100000000)  // is erpm packet (4th bit is 1 or all four bits are 0)
            {
                // update internal values
                if (packetType) *packetType = TELEM_TYPE_ERPM;

                // update output pointer
                *value =
                  erpmToRpm(decode_eRPM_telemetry_value(frameData), dshot_config.num_motor_poles);

            } else  // is extended telemetry packet
            {
                if (packetType)
                    *packetType =
                      (extended_telem_type_t)((frameData >> 8) & 0b1111);  // we return this value
                                                                           // anyway, so we reuse it
...
float DShotRMT::convert_packet_to_volts(uint8_t value) { return (float)value * 0.25; }
...
float DShotRMT::get_telem_success_rate() {
    return (error_packets && successful_packets)
             ? (float)successful_packets / (float)(error_packets + successful_packets)
             : 0.0;
}
...
/// (we don't need to return anything because we fiddle with the class-baked array, accessible
/// everywhere)
