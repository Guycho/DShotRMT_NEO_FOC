...
constexpr auto DSHOT_CLK_DIVIDER =
  8;  // Slow down RMT clock to 0.1 microseconds / 100 nanoseconds per cycle
constexpr auto DSHOT_PACKET_LENGTH = 17;  // Last pack is the pause
constexpr auto DSHOT_THROTTLE_MIN = 48;   // the smallest value considered to be explicitly a
                                          // "throttle" value (smaller values are in the enum below)
constexpr auto DSHOT_THROTTLE_MAX =
  2047;  // the largest value that can be passed back in a single packet
...
// see https://github.com/betaflight/betaflight/blob/master/src/main/drivers/dshot_command.h //list
// of commands and
// https://github.com/betaflight/betaflight/blob/master/src/main/drivers/dshot_command.c#L195 //what
// commands need repeated/delayed and
// https://github.com/betaflight/betaflight/blob/master/src/main/drivers/dshot_command.c#L45 //delay
// timings and https://github.com/bird-sanctuary/extended-dshot-telemetry //dshot docs for bluejay
// ESC

/*
  DshotSettingRequest (KISS24). Spin direction, 3d and save Settings require 10 requests.. and the
...
typedef enum dshot_cmd_e {
    ...
    DSHOT_CMD_SIGNAL_LINE_TELEMETRY_DISABLE,  // Need 6x, no wait required. Disables commands 42 to
                                              // 47
    DSHOT_CMD_SIGNAL_LINE_TELEMETRY_ENABLE,  // Need 6x, no wait required. Enables commands 42 to 47
    DSHOT_CMD_SIGNAL_LINE_CONTINUOUS_ERPM_TELEMETRY,  // Need 6x, no wait required. Enables commands
                                                      // 42 to 47, and sends erpm if normal Dshot
                                                      // frame
    DSHOT_CMD_SIGNAL_LINE_CONTINUOUS_ERPM_PERIOD_TELEMETRY,  // Need 6x, no wait required. Enables
                                                             // commands 42 to 47, and sends erpm
...
typedef enum dshot_mode_e { DSHOT_OFF, DSHOT150, DSHOT300, DSHOT600, DSHOT1200 } dshot_mode_t;
...
static const char* const dshot_mode_name[] = {"DSHOT_OFF", "DSHOT150", "DSHOT300", "DSHOT600",
  "DSHOT1200"};
...
// holds the actual frame data received from the ESC, this is what's passed back and forth through
// the xQueue
...
typedef struct dshot_config_s {
    ...
    uint16_t num_motor_poles;  // how many magnets does the brushless motor have? (essential for
                               // calculating RPM, eRPM = x * RPM)
    ...
    tx_callback_datapack_t tx_callback_datapack;  // a collection of data telling the tx callback
                                                  // how to set up the rx session

    rx_callback_datapack_t rx_callback_datapack;  // collection of settings passed to the rx
                                                  // callback
    ...
    // in the dshot RMT example on the official espidf page uses a fancier encoder, but
    // pre-generating the data and using a simple copy encoder is far easier to track
...
typedef struct foc_phases_output_t {
    uint16_t phase_a;
    uint16_t phase_b;
    uint16_t phase_c;
} foc_phases_output_t;
...
class DShotRMT {
   ...
   private:
    ...
    // new data will be dumped in here from the callback (size doesn't need to be exact, we
    // make it bigger to be safe) (I think it isn't thread safe to touch this while RMT
    // callbacks are
    // running)
